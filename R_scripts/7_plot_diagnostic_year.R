...
sim_long <- sim_long %>% filter(year(Date)== 2019)
obs_long <- obs_long %>% filter(year(Date)== 2019)
#######################
...
ggsave(filename = paste0( output_dir,"/"  ,"plot_diagnostics_", the_year,".png"),
